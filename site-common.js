(function () {
  ...
  async function mountMiniProgramEntry(client) {
    if (document.getElementById('miniProgramEntry')) return
    let entry=null
    try { entry=(await client.from('mini_program_entries').select('*').eq('entry_type','home').maybeSingle()).data } catch (_) {}
    ...
    const wrap=document.createElement('div');wrap.id='miniProgramEntry';wrap.innerHTML='<button class="mp-fab" type="button" aria-label="前往小程序使用更多功能"><span class="mp-ad-icon">✨</span><span class="mp-ad-copy"><span class="mp-ad-label">更多功能</span><span class="mp-ad-title">前往小程序</span></span><span class="mp-ad-arrow">›</span></button><div class="mp-mask"><div class="mp-card"><h2>更多功能，尽在小程序</h2><p>互助广场与“一起来玩哈皮吗”小程序数据互通。扫码进入小程序，还可以使用资料查询、计算器和养成规划等完整功能。</p><div class="mp-qr-wrap"></div><div class="mp-actions"></div></div></div>';document.body.appendChild(wrap)
    const mask=wrap.querySelector('.mp-mask'),actions=wrap.querySelector('.mp-actions'),qrWrap=wrap.querySelector('.mp-qr-wrap')
    const qrFallback=()=>{qrWrap.innerHTML='';const p=document.createElement('p');p.textContent='小程序码尚未配置，请在微信搜索“一起来玩哈皮吗”。';qrWrap.appendChild(p)}
    const img=document.createElement('img');img.className='mp-qr';img.alt='一起来玩哈皮吗小程序码';img.src=entry?.qr_url||'miniprogram-code.png';img.onerror=()=>{if(entry?.qr_url&&img.src.indexOf('miniprogram-code.png')<0){img.src='miniprogram-code.png'}else{qrFallback()}};qrWrap.appendChild(img)
    if(entry&&entry.url_link){const a=document.createElement('a');a.className='mp-open';a.href=entry.url_link;a.textContent='前往小程序';actions.appendChild(a)}
